var thesaDisplaySelector = "span[data-cvoc-protocol='thesa']";
var thesaInputSelector = "input[data-cvoc-protocol='thesa']";
...
function expandthesa() {
    // Check each selected element
    $(thesaDisplaySelector).each(function() {
            ...
            if (id.startsWith("http")) {
                $.ajax({
                    ...
                    success: function(term, status) {
                        ...
                        //For thesa, the 'graph' element contains an array of objects. The one with a uri matching the id describes the term itself (versus braoader/narrower ones, etc.)
...
function updatethesaInputs() {
    // For each input element within thesaInputSelector elements
    $(thesaInputSelector).each(function() {
        var thesaInput = this;
        //If we haven't modified this input yet
        if (!thesaInput.hasAttribute('data-thesa')) {
            ...
            let cvocUrl = $(thesaInput).attr('data-cvoc-service-url');
            let lang = thesaInput.hasAttribute("lang") ? $(thesaInput).attr('lang') : "";
            let langParam = thesaInput.hasAttribute("lang") ? "&lang=" + $(thesaInput).attr('lang') : "";
            let vocabs = JSON.parse($(thesaInput).attr('data-cvoc-vocabs'));
            let managedFields = JSON.parse($(thesaInput).attr('data-cvoc-managedfields'));
            let parentField = $(thesaInput).attr('data-cvoc-parent');
            let termParentUri = $(thesaInput).attr('data-cvoc-filter');
            let allowFreeText = $(thesaInput).attr('data-cvoc-allowfreetext');
            let placeholder = thesaInput.hasAttribute("data-cvoc-placeholder") ? $(thesaInput).attr('data-cvoc-placeholder') : "Select a term";
            let selectId = "thesaAddSelect_" + num;
            //Pick the first entry as the default to start with when there is more than one vocab
            let vocab = Object.keys(vocabs)[0];

            //Mark this field as processed
            $(thesaInput).attr('data-thesa', num);
            ...
            if ($("[data-cvoc-parentfield='" + parentField + "']").length > 0) {
                ...
                anchorSib = $(thesaInput).parentsUntil("[data-cvoc-parentfield='" + parentField + "']").last();
            }
            ...
            var vocabId = "thesaVocabSelect_" + num;
            ...
            if (Object.keys(vocabs).length == 1 || $(thesaInput).val().startsWith("http")) {
            ...
            $("#" + selectId).select2({
                ...
                allowClear: true,
                ajax: {
                    //Call the specified thesa service to get matching terms
                    ...
                    data: function(params) {
                        ...
                        term = params.term;
                        //Add the query - thesa needs a trailing * to match words starting with the supplied letters
            ...
            $('#' + selectId).on('select2:select', function(e) {
                ...
                $("input[data-thesa='" + num + "']").val(data.id);
                ...
                if ($("[data-cvoc-parentfield='" + parentField + "']").length > 0) {
                    var parent = $("input[data-thesa='" + num + "']").closest("[data-cvoc-parentfield='" + parentField + "']");
            ...
            $('#' + selectId).on('select2:clear', function(e) {
                $("input[data-thesa='" + num + "']").attr('value', '');
                $('#' + selectId).text('');
